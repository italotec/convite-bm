// ISOLATED-world relay: the only bridge between content/bridge.js (MAIN world, page session
// tokens) and the extension's chrome.* APIs (tab/tab.js, background.js). MAIN and ISOLATED worlds
// share the DOM but not a JS global, so window.postMessage is the only channel to bridge.js;
// chrome.runtime messaging is the only channel to the rest of the extension.
(function () {
  const MSG_TO_BRIDGE = "cb-relay";
  const MSG_FROM_BRIDGE = "cb-bridge";

  let lastContext = { businessId: null };
